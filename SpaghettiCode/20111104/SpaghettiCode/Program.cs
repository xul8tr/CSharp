...
namespace SpaghettiCode
{
    class Program
    {
        ...
        start:
            ...
            switch (args.Length)
            {
                ...
                case 1:
                    ...
                    {
                        i = WriteAndRespond(beginchar, charlist, stringBuilder, stringlist, step, flattenstring, 500);
                    }
                    break;
                default:
            ...
            {
                i = WriteAndRespond(beginchar, newcharlist, newstringBuilder, newstringlist, step, questiontext, 100, ConsoleKey.N, ConsoleKey.Y);
                switch (i.Key)
                {
                    case ConsoleKey.Y:
                    ...
                    case ConsoleKey.N:
                        ...
                        if (questiontext != null)
                        {
                            i = WriteAndRespond(beginchar, newcharlist, newstringBuilder, newstringlist, step,
                                                questiontext, 100, ConsoleKey.N, ConsoleKey.Y);
                            switch (i.Key)
                            {
                                case ConsoleKey.Y:
                                    newcharlist.Clear();
        ...
        private static ConsoleKeyInfo WriteAndRespond(int beginchar, List<char> charlist, StringBuilder stringBuilder, List<string> stringlist, int step, string flattenstring, int sleep, ConsoleKey consoleKey, ConsoleKey consoleKey2)
        {
            string holderstring;
            ConsoleKeyInfo i;
            ...
            holderstring = stringBuilder.ToString(0, stringBuilder.Length);
            stringlist.Add(holderstring);
            WriteChar(beginchar, stringlist, step, sleep);
            i = GetKey(charlist, consoleKey, consoleKey2);
            return i;
        }

        private static ConsoleKeyInfo WriteAndRespond(int beginchar, List<char> charlist, StringBuilder stringBuilder, List<string> stringlist, int step, string flattenstring, int sleep)
        {
            return WriteAndRespond(beginchar, charlist, stringBuilder, stringlist, step, flattenstring, sleep, ConsoleKey.Enter,
                                   ConsoleKey.Enter);
        }
